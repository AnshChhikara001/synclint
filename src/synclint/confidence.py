...
decide which shapes of change are eligible at all, and the model's confidence
is asked for only inside that gate. A change of any other shape is flagged however
...
from synclint.repair import question_about
from synclint.sections import Section

# Set before any confidence was recorded, so the corpus could not have picked it.
...
# right. The calibration table the harness prints says whether that holds.
...
def outside_reason(change: ChunkChange) -> str:
...
def rate(
    ...
    """The model's confidence in a repair, asked for only inside the gate.

    Raises `SpendCeilingExceeded` rather than make a call past the ceiling.
    """
    answer = json.loads(
        model.complete(
            _RATE, question_about(section, change, explanation, repaired), _RATE_SCHEMA
        )
    )
...
def _touched(before: _Function, after: _Function) -> str:
    parts = [
        ...
        for part, was, now in (
            ...
            ("type parameters", before.type_params, after.type_params),
    ...
    if not parts:
        # Every part a function has is listed above, so this is a grammar
        # newer than these rules; say so rather than name the wrong part.
        return "definition in a way these rules do not name"
    if len(parts) == 1:
        return parts[0]
...
def _dump(
    node: ast.AST | list[ast.stmt] | list[ast.expr] | list[ast.type_param] | None,
) -> str:
    if node is None:
        return ""
